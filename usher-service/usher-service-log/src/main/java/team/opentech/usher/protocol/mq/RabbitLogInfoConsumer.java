 ...
 */
public class RabbitLogInfoConsumer implements MQTTConsumer {
    ...
    public RabbitLogInfoConsumer() {
        this.bus = SpringUtil.getBean(BusInterface.class);
        ...
        traceDetailService = SpringUtil.getBean(TraceDetailService.class);
        traceInfoService = SpringUtil.getBean(TraceInfoService.class);
        traceLogService = SpringUtil.getBean(TraceLogService.class);
    }
    ...
    public void receive(MQMessage message) {
        executor.execute(() -> {
            ...
            try {
                LogDetailTypeEnum parse = LogDetailTypeEnum.parse(text.charAt(0)).orElseThrow(() -> Asserts.makeException("日志标志对应类型不存在:{}", text));
                switch (parse) {
                    case DETAIL:
    ...
    @Override
    public String topic() {
        return LogInfoSendMqUtil.LOG_TOPIC;
    }
