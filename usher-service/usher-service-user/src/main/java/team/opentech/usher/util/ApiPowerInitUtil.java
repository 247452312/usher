    ...
    public static List<PowerInfo> getPowersSingle() throws IOException {
        ...
        Arrays.stream(files).forEach(t -> {
            if (t.exists() && t.isDirectory()) {
                try {
                    String servicePackagePath = t.getCanonicalPath() + "/src/main/java/team/opentech/usher/protocol/rpc";
                    ...
                    Arrays.stream(serviceFiles).forEach(p -> {
                        try {
                            ...
                            String javaClass = "team.opentech.usher.protocol.rpc." + interfaceName;
                            ...
                            Class<?> aClass = Class.forName("team.opentech.usher.protocol.rpc.base.DTOProvider");
                            //父类全部方法名称
