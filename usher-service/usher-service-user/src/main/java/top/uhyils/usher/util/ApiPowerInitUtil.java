    ...
    public static List<PowerInfo> getPowersSingle() throws IOException {
        ...
        Arrays.stream(files).forEach(t -> {
            if (t.exists() && t.isDirectory()) {
                try {
                    String servicePackagePath = t.getCanonicalPath() + "/src/main/java/top/uhyils/usher/protocol/rpc";
                    ...
                    Arrays.stream(serviceFiles).forEach(p -> {
                        try {
                            ...
                            String javaClass = "top.uhyils.usher.protocol.rpc." + interfaceName;
                            ...
                            Class<?> aClass = Class.forName("top.uhyils.usher.protocol.rpc.base.DTOProvider");
                            //父类全部方法名称
