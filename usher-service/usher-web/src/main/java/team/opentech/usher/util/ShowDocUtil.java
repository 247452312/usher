...
public class ShowDocUtil {
    ...
    public static void initDoc() throws Exception {
        ...
        String courseFile = directory.getCanonicalPath() + "/usher-api";
        File apiDir = new File(courseFile);
        File[] files = apiDir.listFiles();
        for (File projectDir : files) {
            team.opentech.usher.util.LogUtil.info(ShowDocUtil.class, "最外层: " + projectDir.getPath());
            if (projectDir.isDirectory()) {
                String s = projectDir.getPath() + "/src/main/java/team/opentech/usher/service";
                File[] services = new File(s).listFiles();
                for (File service : services) {
                    team.opentech.usher.util.LogUtil.info(ShowDocUtil.class, "中层的service: " + service.getPath());
                    if (service.exists() && service.isFile()) {
                        String path = service.getPath();
                        String substring = path.substring(path.lastIndexOf("\\") + 1, path.lastIndexOf("."));
                        Class<?> aClass = Class.forName("team.opentech.usher.service." + substring);
                        Method[] declaredMethods = aClass.getDeclaredMethods();
                        for (Method declaredMethod : declaredMethods) {
                            team.opentech.usher.util.LogUtil.info(ShowDocUtil.class, "内层的method: " + declaredMethod.getName());
    ...
    private static void sendHttpToShowDoc(JSONObject data) throws Exception {
        ...
        Object o = team.opentech.usher.util.HttpUtil.sendHttpPost(url, head, data);
        team.opentech.usher.util.LogUtil.info(o.toString());
    }
    ...
    private static String getList(String paramJson) {
        ...
        try {
            ...
            team.opentech.usher.util.LogUtil.info(paramJson);
            team.opentech.usher.util.LogUtil.error(ShowDocUtil.class, e);
        }
