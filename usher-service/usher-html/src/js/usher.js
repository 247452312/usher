...
function pushRequest(interfaceName, methodName, data, success, target = false) {
  ...
  let json = JSON.stringify(
      new usherrequest(getAttrBySession("token").token, interfaceName, methodName,
          data));
  console.log("发送");
  console.log(json);
  $.ajax({
...
function getDict(dictCode) {
  pushRequest("DictService", "getDictByCode", {code: dictCode},
      function (data) {
        return data;
      })
}
...
function bandUpFile(element, success) {
  $(element).change(function () {
    ...
    reader.onload = function (e) {
      let json = JSON.stringify(
          new usherrequest(getAttrBySession("token").token, null, null,
        ...
        top.layer.prompt({
          ...
          success: function (layero, index) {
            ...
            layero.on('click', '#verifyImg', function () {
              pushRequest('VerificationService', 'getVerification', {},
                  function (data) {
                    key = data.key;
          ...
          let code = value;
          pushRequest('VerificationService', 'verification',
              {code: code, key: key}, function (data) {
                if (data) {
