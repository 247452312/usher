...
class JavaAstUtilTest {
    ...
    void integrationCompilationUnit() throws FileNotFoundException {
        LogUtil.info("解析文件开始");
        List<CompilationUnit> compilationUnits = JavaAstUtil.loadProjectPath("F:\\share\\usher");
        LogUtil.info("解析文件完成,分析文件开始");
        JavaAstUtil.integrationCompilationUnit(compilationUnits);

        TypeDeclaration<?> typeDeclaration = AstContext.getAllCompilationUnitMap().get("com.github.javaparser.ast.stmt.Statement");
